...
(
  ...
  always_ff @(posedge clk) begin
    if (ld_a_en) a_mem[ld_a_row][ld_a_k] <= ld_a_data;
    if (ld_b_en) b_mem[ld_b_k][ld_b_col] <= ld_b_data;
  end
  ...
  always_comb begin
    for (int i = 0; i < ARRAY_ROWS; i++)
      a_unskewed[i] = feed_valid ? a_mem[i][k_idx] : '0;
    for (int j = 0; j < ARRAY_COLS; j++)
      b_unskewed[j] = feed_valid ? b_mem[k_idx][j] : '0;
  end
  ...
  systolic_array u_array (
    .clk(clk), .rst_n(rst_n),
    .en(array_en), .clear_acc(clear_acc),
